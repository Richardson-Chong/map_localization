...
//calculate d(Iq,Ip) with 2 scans
...
std::pair<double, int> SCManager::distanceBtnScanContext( MatrixXd &_sc1, MatrixXd &_sc2 )
{
    ...
    //using mean value of each col to get a Sector Key
    MatrixXd vkey_sc1 = makeSectorkeyFromScancontext( _sc1 );
    MatrixXd vkey_sc2 = makeSectorkeyFromScancontext( _sc2 );
    //using the 2-poly dist of the Sector_key Matrix(1,N_sect)
...
//create the ring key from each row data mean of every Scan Context 
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    //mean of each ring of the Matrix
...
bool SCManager::detectLoopClosureID (pcl::PointCloud<SCPointType> &scan, Eigen::Matrix4f &pose )
{
    Eigen::MatrixXd SCquery = makeScancontext(scan); // v1 
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext( SCquery );
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext( SCquery );
    std::vector<float> polarcontext_invkey_vec = eig2stdvec( ringkey );


    auto curr_key = polarcontext_invkey_vec; // current observation (query)
    //mean of each ring of the Matrix
    auto curr_desc = SCquery; // current observation (query)
    ...
    // To do: return also nn_align (i.e., yaw diff)
    float yaw_diff_rad = deg2rad(nn_align * PC_UNIT_SECTORANGLE);
    Eigen::AngleAxisf orientation_change(yaw_diff_rad, Eigen::Vector3f::UnitZ());
