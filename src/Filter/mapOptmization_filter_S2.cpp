...
public:
    ...
    Eigen::MatrixXf G_t;
    Eigen::MatrixXf J_t;
    Eigen::MatrixXf Jt_inv;
    Eigen::MatrixXf P_t;
    Eigen::MatrixXf Imat;
    Eigen::MatrixXf P_t_inv;
    ...
    Eigen::MatrixXf R;
    Eigen::MatrixXf H_k;
    Eigen::MatrixXf H_k_T_R_inv;
    Eigen::MatrixXf K_k;
    Eigen::MatrixXf updateVec_;
    Eigen::MatrixXf errVec_;
    Eigen::MatrixXf HRH;
    Eigen::MatrixXf HRH_inv;
    //states
    ...
    void statePredict()
    {
        ...
        if(!USE_S2)
            ...
            un_acc_last = R_transformTobeMapped*(lastImu.first-filterState.ba_)+filterState.Gs2.get_vect();
        ...
        for(size_t i=1; i<imuBucket.size(); i++)
        {
            ...
            if(!USE_S2)
                ...
                un_acc_next = R_transformTobeMapped*(thisImu.first-filterState.ba_)+filterState.Gs2.get_vect();
        ...
        // ROS_INFO_STREAM("Pt in prediction is:"<<endl<<P_t<<endl);
    ...
    bool updateTransformationIESKF(Eigen::Vector3f& g_vec)
    {
        ...
        // float sqrt_lidar_std = sqrt(LIDAR_STD);
        ...
        if(USE_S2)
        {
            float len = filterState.Gs2.length;
            Eigen::Vector2f dg = errVec_.block<2, 1>(GW_, 0);
            Eigen::Matrix<float, 3, 2> Bxj, Bx0;
            intermediateState.Gs2.S2_Bx(Bxj);
            filterState.Gs2.S2_Bx(Bx0);
            
            J_t.block<2, 2>(GW_, GW_) = -1 / (len * len)
                                      * (Bxj.transpose() * Sophus::SO3f::exp(Bx0*dg).matrix()
                                      * GetSkewMatrix(filterState.Gs2.get_vect())*GetSkewMatrix(filterState.Gs2.get_vect())
                                      * Amatrix(Bx0*dg).transpose() * Bx0);

            // ROS_INFO_STREAM("len: "<<endl<<len<<endl<<endl);
            // ROS_INFO_STREAM("Bxj: "<<endl<<Bxj<<endl<<endl);
            // ROS_INFO_STREAM("Bx0: "<<endl<<Bx0<<endl<<endl);
            // ROS_INFO_STREAM("J_t: "<<endl<<J_t<<endl<<endl);
        }

        //calculate K_j
        Jt_inv.block<3, 3>(ROT_, ROT_) = (J_t.block<3, 3>(ROT_, ROT_)).inverse();
        if(USE_S2)
            Jt_inv.block<2, 2>(GW_, GW_) = (J_t.block<2, 2>(GW_, GW_)).inverse();
        ...
        if(USE_S2) g_vec = intermediateState.Gs2.get_vect();
    ...
    void updateState(const Eigen::Vector3f& g_vec, double dt)
    {
        TicToc t_1;
        ...
        if(USE_S2)
        {
            float len = intermediateState.Gs2.length;
            Eigen::Vector2f dg = updateVec_.block<2, 1>(GW_, 0);
            Eigen::Matrix<float, 3, 2> Bxj, Bx0;
            intermediateState.Gs2.S2_Bx(Bxj);
            S2<float> Gk(g_vec);
            Gk.S2_Bx(Bx0);
            J_t.block<2, 2>(GW_, GW_) = -1 / (len * len)
                                      * (Bxj.transpose() * Sophus::SO3f::exp(Bx0*dg).matrix()
                                      * GetSkewMatrix(Gk.get_vect())*GetSkewMatrix(Gk.get_vect())
                                      * Amatrix(Bx0*dg).transpose() * Bx0);
        }
        ...
        if(!USE_S2)
           ...
           cout<<"更新后的states: "<<endl
                                ...
                                <<filterState.ba_.transpose()<<" "<<endl
                                <<filterState.bw_.transpose()<<" "<<endl
                                <<filterState.Gs2.get_vect().transpose()<<" "<<endl
                                <<endl; 
        // ROS_INFO_STREAM("Pt in update is:"<<endl<<P_t<<endl);
    ...
    void updateStatebyIeskf(double dt)
    {
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            {   
                cout<<"**********第"<<frame_count<<"帧的第"<<iterCount<<"次优化**********"<<endl;
                ...
                //get Gvec before kth update for G's tangent space transition
                Eigen::Vector3f g_vec;
                converge = updateTransformationIESKF(g_vec);
...
int main(int argc, char** argv)
{
    ...
    ROS_INFO("\033[1;32m----> Map Filter_S2 Optimization Started.\033[0m");
