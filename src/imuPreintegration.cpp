...
public:
    ...
    //FOR CHANG'AN
    ros::Subscriber subGPSfactoradd;
    ros::Subscriber subInitialLLA;
    bool GPSfacotradd = false;
    ros::Publisher pubGPSodom;
    Eigen::Vector3d lla;
    Eigen::Vector3d lla_desinged;
    ...
    void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        file_imu.setf(std::ios::fixed, std::_S_floatfield);
        file_imu << laserOdometry.header.stamp.toSec() << " " 
...
public:
    ...
    gtsam::Pose3 lidar2Imu = gtsam::Pose3(gtsam::Rot3(q.w(), q.x(), q.y(), q.z()), gtsam::Point3(extTrans.x(), extTrans.y(), extTrans.z()));
    ...
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        if (key == 100)
        {
            ...
            // std::cout<<updatedPoseNoise->covariance()<<endl<<endl;
            gtsam::noiseModel::Gaussian::shared_ptr updatedVelNoise  = gtsam::noiseModel::Gaussian::Covariance(optimizer.marginalCovariance(V(key-1)));
            // std::cout<<updatedVelNoise->covariance()<<endl<<endl;
        ...
        // graphFactors.add(gtsam::BetweenFactor<gtsam::imuBias::ConstantBias>(B(key - 1), B(key), gtsam::imuBias::ConstantBias(),
        //                  gtsam::noiseModel::Diagonal::Sigmas(sqrt(imuIntegratorOpt_->deltaTij()) * noiseModelBetweenBias.array().sqrt().matrix())));
        // add pose factor
        //消除lever arm的方法
    ...
    void publishImuBias(gtsam::imuBias::ConstantBias& bias, double timestamp)
    {
        ...
        ROS_INFO("BIAS states has been publisherd!");
        ROS_INFO_STREAM("Acc bias: "<<bias.vector()(0)<<" "<<bias.vector()(1)<<" "<<bias.vector()(2)<<endl);
        ROS_INFO_STREAM("Gyro bias: "<<bias.vector()(3)<<" "<<bias.vector()(4)<<" "<<bias.vector()(5)<<endl);
    }
